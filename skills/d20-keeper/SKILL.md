...
The daemon is outbound-only. Epoch publication and fulfillment share one wallet nonce lane; HTTP work cannot sign. Use a dedicated transaction wallet and separate VRF key. A primary keeper's transaction wallet must be the registry `committer()`. A follower keeper (`KEEPER_ROLE=follower`) runs on another host with its own wallet, journal and host-local locks and the same VRF key; its wallet must be an allowed backup committer, and it works the newest end of the queue and only once the join rule fires: the oldest unserved request is older than `FOLLOWER_DELAY_SECONDS`, the visible queue is deeper than `FOLLOWER_QUEUE_JOIN`, or the primary is chain-dead (work waiting `PRIMARY_LIVENESS_SECONDS` without the committer's confirmed nonce advancing). Every node sends a request within 20 seconds of its deadline whatever its role, and `FOLLOWER_LANES`/`FOLLOWER_RANK` split the tail between several followers. A follower earns the keeper share of the requests it serves itself, and still needs its own gas funding. Core variable names are generic: RPC_URLS, CHAIN_ID, COORDINATOR_ADDRESS, KEEPER_DB, TX_KEY_FILE, VRF_KEY_FILE and SEND_TRANSACTIONS. CHAIN_ID is required explicitly; no Arc default is assumed. Keep sends disabled until authorized.
...
- Keeper fee share goes to the wallet that submitted the accepted proof when the registry authorizes it (`isAuthorizedCommitter`: the committer or an allowed backup committer), and to `committer()` for any other submitter; failed payment is keeper credit. Request refunds and callback retries are separate authorized operations.
